...
export function ShopCatalogGrid({ products }: { products: ShopCardProduct[] }) {
  const reduceMotion = useReducedMotion();
  const signature = products.map((product) => product.item.productId).join(",");
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 md:gap-5 lg:gap-6">
        {shown.map((product, index) => {
          ...
          return (
            ...
            >
              <ShopProductCard product={product} variant="grid" />
            </motion.div>
          );
        })}
      </div>
